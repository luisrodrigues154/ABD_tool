...
SEXP attribute_hidden do_seq(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    */
	...
	else if (miss_to)
	{
		...
		if (rby == (int)rby && rfrom == (int)rfrom && rfrom <= INT_MAX && rfrom >= INT_MIN && rto <= INT_MAX && rto >= INT_MIN)
		{
			ans = allocVector(INTSXP, lout);
			for (i = 0; i < lout; i++)
			{
	...
	else if (miss_from)
	{
		...
		if (!R_FINITE(rby))
			errorcall(call, _("'%s' must be a finite number"), "by");
		if (rby == (int)rby && rto == (int)rto && rfrom <= INT_MAX && rfrom >= INT_MIN && rto <= INT_MAX && rto >= INT_MIN)
		{
			ans = allocVector(INTSXP, lout);
			for (i = 0; i < lout; i++)
			{

...
LogicalAnswer(SEXP x, struct BindData *data, SEXP call)
{
	...
	case LGLSXP:
		for (i = 0; i < XLENGTH(x); i++)
			LOGICAL(data->ans_ptr)
			[data->ans_length++] = LOGICAL(x)[i];
		break;
	case INTSXP:
		for (i = 0; i < XLENGTH(x); i++)
		{
			int v = INTEGER(x)[i];
			LOGICAL(data->ans_ptr)
			[data->ans_length++] = (v == NA_INTEGER) ? NA_LOGICAL : (v != 0);
		}
		break;
	case RAWSXP:
		for (i = 0; i < XLENGTH(x); i++)
			LOGICAL(data->ans_ptr)
			[data->ans_length++] = (int)RAW(x)[i] != 0;
		break;
	default:
...
IntegerAnswer(SEXP x, struct BindData *data, SEXP call)
{
	...
	case LGLSXP:
		for (i = 0; i < XLENGTH(x); i++)
			INTEGER(data->ans_ptr)
			[data->ans_length++] = LOGICAL(x)[i];
		break;
	case INTSXP:
		for (i = 0; i < XLENGTH(x); i++)
			INTEGER(data->ans_ptr)
			[data->ans_length++] = INTEGER(x)[i];
		break;
	case RAWSXP:
		for (i = 0; i < XLENGTH(x); i++)
			INTEGER(data->ans_ptr)
			[data->ans_length++] = (int)RAW(x)[i];
		break;
	default:
		errorcall(call, _("type '%s' is unimplemented in '%s'"),
...
ComplexAnswer(SEXP x, struct BindData *data, SEXP call)
{
	...
	case REALSXP:
		for (i = 0; i < XLENGTH(x); i++)
		{
			COMPLEX(data->ans_ptr)
			[data->ans_length].r = REAL(x)[i];
			COMPLEX(data->ans_ptr)
			[data->ans_length].i = 0.0;
			data->ans_length++;
		}
		break;
	case CPLXSXP:
	...
	case LGLSXP:
		for (i = 0; i < XLENGTH(x); i++)
		{
			...
			{
				COMPLEX(data->ans_ptr)
				[data->ans_length].r = NA_REAL;
				COMPLEX(data->ans_ptr)
				[data->ans_length].i = NA_REAL;
			}
			else
			{
				COMPLEX(data->ans_ptr)
				[data->ans_length].r = xi;
				COMPLEX(data->ans_ptr)
				[data->ans_length].i = 0.0;
			}
			data->ans_length++;
			...
			if (isMatrix(u) || length(u) >= lenmin)
			{
				...
				if (TYPEOF(u) <= INTSXP)
				{
					...
					else
					{
						FILL_MATRIX_ITERATE(n, rows, idx, cols, k)
						REAL(result)
						[didx] = (INTEGER(u)[sidx]) == NA_INTEGER ? NA_REAL : INTEGER(u)[sidx];
						n += idx;
					}
				}
				else if (TYPEOF(u) == REALSXP)
				...
				{ /* RAWSXP */
					if (mode == LGLSXP)
					{
						FILL_MATRIX_ITERATE(n, rows, idx, cols, k)
						LOGICAL(result)
						[didx] = RAW(u)[sidx] ? TRUE : FALSE;
					}
					else
						FILL_MATRIX_ITERATE(n, rows, idx, cols, k)
					INTEGER(result)
					[didx] = (unsigned char)RAW(u)[sidx];
				}
			}
		}
	}

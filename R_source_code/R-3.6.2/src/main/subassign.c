...
do_subassign2_dflt(SEXP call, SEXP op, SEXP args, SEXP rho)
{
	...
	if (isVector(x))
	{
		...
		else
		{
			...
			{
				pindx[i] = (int)
					get1index(CAR(subs), isNull(names) ? R_NilValue : VECTOR_ELT(names, i),
							  pdims[i],

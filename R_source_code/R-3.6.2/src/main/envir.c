...
attribute_hidden
    SEXP
    R_GetVarLocValue(R_varloc_t vl)
{
    SEXP cell = vl.cell;
...
attribute_hidden
    SEXP
    R_GetVarLocSymbol(R_varloc_t vl)
{
    return TAG(vl.cell);
}
...
attribute_hidden void readS3VarsFromFrame(SEXP rho,
...
attribute_hidden
    SEXP
    ddfindVar(SEXP symbol, SEXP rho)
{
    int i = ddVal(symbol);
    return ddfind(i, rho);
}
...
attribute_hidden
    SEXP
    findFun3(SEXP symbol, SEXP rho, SEXP call)
{
    SEXP vl;
...
havebinding:
    ...
    else
    {
        PROTECT(rval);
        LOGICAL(rval)
        [0] = R_isMissing(PREXPR(t), PRENV(t));
        UNPROTECT(1);
    }
    return rval;
}
...
#define NONEMPTY_(_FRAME_)        \
    CHAR(PRINTNAME(TAG(_FRAME_))) \
            [0] != '.' &&         \
        CAR(_FRAME_) != R_UnboundValue

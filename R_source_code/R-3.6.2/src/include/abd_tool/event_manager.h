...
void makeStrForExp(IF_EXPRESSION * newExpr, char * stmtStr){
    if(newExpr->left_type == IF_EXPR){
        ...
        if(obj->id == -1){
            ...
            sprintf(stmtStr, "%s%s", stmtStr, obj->name);
    }
